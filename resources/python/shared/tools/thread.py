...
def getFromThreadScope(target_thread, object_name):
	...
	assert target_thread.getState() != Thread.State.TERMINATED, "Thread already dead: %r" % target_thread
...
def getThreadRootFrame(target_thread):
	"""Get the original calling context of a thread."""
	frame = getThreadFrame(target_thread)
	# traverse the stack
	while frame.f_back:
		frame = frame.f_back
	return frame
